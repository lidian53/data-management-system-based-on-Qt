...
void Widget::on_originpwdpushButton_clicked()
{
    QSqlQuery query;
    query.exec("select pwd from password");
    query.next();
    if(query.value(0).toString()==ui->OriginpwdLineEdit->text()){
...
void Widget::on_newpwdBtn_clicked()
{
    QSqlQuery query;
    query.exec("select pwd from password");
    query.next();
    if(query.value(0).toString()==ui->NewpwdLineEdit->text()){
        QMessageBox::information(this,tr("请输入新密码"),
                                 tr("新密码不能和原密码一致"),QMessageBox::Ok);
        ui->NewpwdLineEdit->setFocus();
    }
    else{
        QSqlDatabase::database().transaction();
        QString newpwd=ui->NewpwdLineEdit->text();
        QString oripwd=query.value(0).toString();
        bool npw=query.exec(QString("update password set pwd='%1' where pwd='%2'")
                          .arg(newpwd).arg(oripwd));
        if(npw){
            QSqlDatabase::database().commit();
